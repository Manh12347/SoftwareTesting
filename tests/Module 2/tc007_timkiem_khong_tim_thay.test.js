...
describe('TC007: Tìm kiếm không có kết quả', function () {
    this.timeout(60000);
    let driver;
    ...
    afterEach(async function () {
        if (this.currentTest.state === 'failed') {
            const screenshotPath = await takeScreenshot(driver, this.currentTest.title, 'tc007');
            addContext(this, {
    ...
    it('Tìm kiếm với từ khóa "123123" - hiển thị thông báo không tìm thấy', async function () {
        ...
        const screenshotPath = await takeScreenshot(driver, this.test.title, 'tc007');
        addContext(this, {
