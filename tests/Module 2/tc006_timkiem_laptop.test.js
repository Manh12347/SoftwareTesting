...
describe('TC006: Tìm kiếm sản phẩm', function () {
    this.timeout(60000);
    let driver;
    ...
    afterEach(async function () {
        if (this.currentTest.state === 'failed') {
            const screenshotPath = await takeScreenshot(driver, this.currentTest.title, 'tc006');
            addContext(this, {
    ...
    it('Tìm kiếm với từ khóa "Laptop" - có ít nhất 1 sản phẩm laptop', async function () {
        ...
        const screenshotPath = await takeScreenshot(driver, this.test.title, 'tc006');
        addContext(this, {
